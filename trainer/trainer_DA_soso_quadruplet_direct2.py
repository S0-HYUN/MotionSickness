    ...
    def training(self, shuffle=True, interval=1000):
        ...
        for e in tqdm(range(self.epoch)):
            ...
            for idx, data in enumerate(data_loader):
                ...
                # rest_clf = rest_clf.cpu().detach().numpy()
                # rest_clf = np.tile(rest_clf, pred.shape[0])
                # rest_clf = torch.tensor(rest_clf).to(self.device)
                # distance_vec = torch.norm(torch.stack([pred, rest_clf]), p = 2, d = pred.shape)
                # distance_vec = torch.norm(torch.dstack([MS_feature, rest_proto_vec]), dim=2)
                ...
                # class0_idx = y==0; class1_idx = y==1; class2_idx = y==2
                # class0_x = pred[class0_idx]; class1_x = pred[class1_idx]; class2_x = pred[class2_idx]
                # count = min(sum(class0_idx), sum(class1_idx), sum(class2_idx))
                # loss_quad = 0
                # for cc in range(count):
                #     loss_sep = self.criterion_quad(class0_x[cc], class1_x[cc], class2_x[cc], rest_proto)
                #     loss_quad += loss_sep
    ...
    def evaluation(self, data, interval=1000, state=None):
        ...
        with torch.no_grad(): # gradient 안함
            ...
            for idx, data in enumerate(data_loader):
                ...
                # distance_vec = torch.norm(torch.stack([pred, rest_clf]), p = 2)
                # distance_vec = torch.sqrt((pred - rest_clf)**2)
                ...
                # loss_quad = self.criterion_quad(pred, true_label, self.device)
                loss_quad = self.criterion_quad(true_label, pred, rest_proto)
