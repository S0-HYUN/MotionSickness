    ...
    def training(self, shuffle=True, interval=1000):
        ...
        for e in tqdm(range(self.epoch)):
            ...
            for idx, data in enumerate(data_loader):
                ...
                x = x.view(self.args.batch_size, 1, self.channel_num, -1) # [1, 1, 25, 750]
    ...
    def evaluation(self, data, interval=1000, state=None):
        ...
        with torch.no_grad(): # gradient 안함
            ...
            for idx, data in enumerate(data_loader):
                ...
                b = x.shape[0]
                if x.shape[0] != self.args.batch_size : continue
                x = x.view(b, 1, self.channel_num, -1)
                ...
                #---# rest data #---# --> 전체 prototype으로....?
                rest_proto = torch.tensor(get_meanPrototype()).to(self.device)
                rest_proto_vec = rest_proto.repeat(pred.shape[0])
                rest_proto_vec = rest_proto_vec.view(self.args.batch_size, -1)
                ...
                # rest_clf = torch.tensor(get_meanPrototype()).to(self.device)
                pred_label = (pred>0.5).cumprod(axis=1).sum(axis=1)-1; pred_label[pred_label<0] = 1
