    ...
    def training(self, shuffle=True, interval=1000):
        ...
        for e in tqdm(range(self.epoch)):
            ...
            for idx, data in enumerate(data_loader):
                ...
                #---# rest data #---#
                random_vec = np.random.randint(0, self.data_r.data[0].shape[0], size=self.args.batch_size)
                ...
                x_rest = self.data_r.data[0][random_vec]
                x_rest = torch.tensor(x_rest.reshape(self.args.batch_size, 1, self.channel_num, -1))
                rest_feature = self.model(x_rest.to(device=self.device).float())#######################
                rest_proto = rest_feature.mean(0)
                rest_proto_vec = rest_proto.repeat(pred.shape[0]).view(self.args.batch_size, -1)

                '''d'''
                rest_clf = self.model(x_rest.to(device=self.device).float(), fc=True) # MLP 거침
