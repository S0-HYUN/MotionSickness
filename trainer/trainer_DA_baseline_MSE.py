...
class TrainMaker:
    ...
    def training(self, shuffle=True, interval=1000):
        ...
        # data_rest_loader = DataLoader(self.data_r, batch_size=self.args.batch, sampler=sampler) #근데 사실 필요없는 게 아닐까.
    ...
    def evaluation(self, data, interval=1000, state=None):
        ...
        with torch.no_grad(): # gradient 안함
            ...
            for idx, data in enumerate(data_loader):
                ...
                pred_label = (pred>0.5).cumprod(axis=1).sum(axis=1)-1 # ; pred_label[pred_label<0] = 1
                loss = self.criterion(pred, y_vector.to(device=self.device))
            ...
            if self.args.mode == "test":
                ...
                create_folder(f"./features/features_{self.args.model}_{self.args.standard}_{self.args.criterion}")
                np.savez(f"./features/features_{self.args.model}_{self.args.standard}_{self.args.criterion}/original_subj{str(self.args.test_subj).zfill(2)}", test_embeds, true_label_acc)
