...
class TrainMaker:
    def __init__(self, args, model, data, data_v=None, data_r=None, data_v_r=None, DA=False):
        ...
        self.criterion_cee = nn.CrossEntropyLoss()
    ...
    def training(self, shuffle=True, interval=1000):
        ...
        for e in tqdm(range(self.epoch)):
            ...
            for idx, data in enumerate(data_loader):
                ...
                # distance_vec = torch.sqrt((pred - rest_feature)**2)
                ...
                loss_cee = self.criterion_cee(pred, y.flatten().long().to(device=self.device))
                ...
                # loss_mse.backward()
                # loss = 0.5 * loss_tri + loss_mse
                loss = 0.5 * loss_tri + loss_cee
                loss.backward()
                epoch_loss += loss
                self.optimizer.step()
    ...
    def evaluation(self, data, interval=1000, state=None):
        ...
        with torch.no_grad(): # gradient 안함
            ...
            for idx, data in enumerate(data_loader):
                ...
                x_rest, y_rest, subj_rest = data_rest_loader.dataset[random_vec]
                x_rest = x_rest.reshape(self.args.batch_size, 1, self.channel_num, -1)
                rest_feature = self.model(x_rest.to(device=self.device).float()) # [128, 800]
                rest_proto = rest_feature.mean(0) # [800]
                ...
                loss_cee = self.criterion_cee(pred, y.flatten().long().to(device=self.device))
                # loss = self.criterion(pred, y.flatten().long().to(device=self.device)) # 원래
                # loss = 0.5 * loss_tri + loss_mse.item()
                loss = 0.5 * loss_tri + loss_cee
                valid_loss += loss
