...
class TrainMaker:
    def __init__(self, args, model, data, data_v=None, data_r=None, data_v_r=None, DA=False):
        ...
        self.criterion_quad = QuadrupletLoss()
    ...
    def training(self, shuffle=True, interval=1000):
        ...
        for e in tqdm(range(self.epoch)):
            ...
            for idx, data in enumerate(data_loader):
                ...
                # 추가
                pred = self.model(x.to(device=self.device).float(), subj.to(self.device), embedding=True)
                ...
                # loss = loss_quad.item() + loss_mse.item()
    ...
    def evaluation(self, data, interval=1000, state=None):
        ...
        with torch.no_grad(): # gradient 안함
            ...
            for idx, data in enumerate(data_loader):
                ...
                pred = self.model(x.to(device=self.device).float(), subj.to(self.device).float())
