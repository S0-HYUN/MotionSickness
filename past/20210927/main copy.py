...
for lr in lr_list :
    for wd in wd_list :
        ...
        acc, f1, cm, loss = trainer1.train(data, data_validation, batch_size=batch_size)
        # last_param = os.listdir('./param/lr{}_wd{}/'.format(lr, wd))[-1] ############################################# 저장 위치
        # best_model = eegnet_models.EEGNet_latest(10, channel_num, one_bundle).to(device=device)
        ...
        # data_test = data_loader.Dataset(path, [test_file], 'test') # 50
        # acc_v, f1_v, cm_v, loss_v = trainer2.prediction(data_test, batch_size, test=True)
        # df.loc[idx] = [test_subj, expt, lr, wd, acc_v, f1_v, loss_v.cpu().numpy()]
        df.loc[idx] = [test_subj, expt, lr, wd, acc, f1, loss]
