...
class Dataset(Dataset) :
    def __init__(self, args, phase, idx):
        ...
        self.x = torch.tensor(self.data[0]); self.x = self.x[idx] 
        # self.y = torch.tensor(self.data[1]).mean(-1); self.y = self.y[idx]
        self.y = torch.tensor(self.data[1]); self.y = self.y.reshape(-1, 1)
        self.y = self.y[idx]
