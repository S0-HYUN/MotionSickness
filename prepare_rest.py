...
class Load_Data() :
    def __init__(self, args) :
        ...
        for i in range(len(datalist)) :
            ...
            #---# save single data #---#  
            #--------------------------#
            output_dir = str(args.output_path) + "/Expt" + str(expt) + "/day" + str(day)
            filename = "subj" + str(subj).zfill(2)
            file_path_name = output_dir + "/" + filename + ".npz"
            check_and_save(output_dir, filename, x)

    def __getitem__(self) :
        return

def check_and_save(out_dir, filename, x):
    if not os.path.isfile(filename):
        save_npzfile(out_dir, filename, x)
    else:
        print(f"{filename} is already exist!")

def save_npzfile(out_dir, filename, x) :
